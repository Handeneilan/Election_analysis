...
with open(file_to_save, "w") as txt_file:
    ...
    for county_name in county_votes.keys():
        ...
        current_county_vote = county_votes[county_name]

        # 6c: Calculate the percentage of votes for the county.
        percentage_vote = float(current_county_vote) / float(total_votes) * 100

        # 6d: Print the county results to the terminal.
        county_results = (
            f"{county_name}: {percentage_vote:.1f}% ({current_county_vote:,})\n"
        )
        ...
        if (current_county_vote > largest_county_voter):
            largest_county_voter = current_county_vote
